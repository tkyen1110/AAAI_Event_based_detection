...
class ConvLSTMCell(RNNCell):
    ...
    def __init__(self, hidden_dim, kernel_size, padding=1, conv_func=nn.Conv2d, hard=False):
...
class ConvRNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1,
        ...
        if cell.lower() == "lstm":
            self.timepool = ConvLSTMCell(out_channels, kernel_size=kernel_size, padding=padding, conv_func=conv_hidden)
        else:
            self.timepool = ConvGRUCell(in_channels, out_channels, kernel_size=kernel_size, conv_func=conv_hidden)
